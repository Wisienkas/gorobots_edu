...
void NGNet::put_incsbox(Cell *isb, int ni /*number input = e.g., 2*/, int no /*number output = e.g., 1*/, double *xp /*input vector*/, double *Ivr /*wbasis vector = width of each input*/, int *nc /*neuron number*/)
{
	int i,j;
	Unit *curr;
...
double NGNet::incsbox_activate( Cell *isb, double *x, int *nc)
{
	...
	for(unum=0;unum<*nc /*e.g., 9*/;unum++,curr++)
	{
		...
		else
		{
			...
			double active_thresh = 0.001; // Threshold of softmax
...
double NGNet:: incsbox_update(Cell *isb, double *x, double *error/*-TDerror*/, double rate, int *nc,
		double *Ivr, double Thr /*e.g. 0.1*/ ,double near /*e.g. 0.6*/)

{
	...
	//-3)  Cal ui[0], ui[1] = actual state-first_initial state -- NOT USE
	for(i=0;i<nin;i++)
		ui[i] = x[i] -  curr->cx[i]; //-----------NOT USE------------------------HELP why??

	//-4) Weight adaptation
	for(unum=0;unum<*nc;unum++,curr++)
	{
		//4.1 - Neural activation of each hidden neuron of RBF
		ac = curr->ac;
		//4.2 - Find max neural activation
		if(ac > maxac)	  maxac=ac;

		//4.3 - Find sum of(-TDerror*weight_i), update weight = (0.5* TDerror* sftmax_output_of hidden neuron or RBF)
		sume = 0.0;
		for(k=0;k<nout /*1*/;k++){

			//sum (-TDerror * weight)
			sume += error[k] /*-TDerror*/ *curr->w[k];

			//wk = (0.5* TDerror* sftmax_output_of hidden neuron or RBF)
			curr->w[k]-=rate*error[k]*(curr->dw[k]); /*update weight*/
			//curr->w[k] +=rate*(-1*error[k])*(curr->dw[k]); /*update weight*/

		}
	}

	/******************Adaptive RBF hidden neurons adding more**********************************************/
	//5 - Find --> TDerror^2, Mean seuared error
	serr = 0.0;
	for(i=0;i<nout;i++)
		serr += error[i]*error[i];

	msqr = sqrt(serr/nout /*nout = 1*/);   /* mean squared error*/

	/*create new basis function*/


	if(msqr > Thr /*e.g. 0.1*/ && maxac < near /*e.g. 0.6*/ && (*nc) < 500 )
	{
		int i,j;
		Unit *ptr;
