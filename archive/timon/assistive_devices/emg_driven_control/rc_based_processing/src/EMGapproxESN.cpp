...
EMGapproxESN::EMGapproxESN(
    ...
    preFiltLeakage          = _preFiltLeakage;
    percentage_1            = _percentage_1;
    percentage_2            = _percentage_2;
    repetition              = _repetition;
    iterationLimit          = _iterationLimit;
    
    // Initialize buffer used in pre-filtering
    for( unsigned int i = 0; i < numberOfInputs; i++ ) {
        
        preFilterPrev.push_back( 0.0 );
    ...
    ESN->NoiseRange         = 0.005;// amplitude of noise
...
double EMGapproxESN::preFilterData( unsigned int selectedInput, double inputValue, double filterLeakage ) {
    
    if( filterLeakage > 0.0 ) {
    
        double actual;
    
        actual = ( 1 - filterLeakage ) * inputValue + filterLeakage * preFilterPrev[selectedInput];

        preFilterPrev[selectedInput] = actual;
    ...
    for( unsigned int r = 0; r < repetition; r++ ) {
        ...
        for( unsigned int i = 0; i < trainingUnits.size(); i++ ) {
            ...
            for( unsigned int j = 0; j < numberOfInputs; j++ ) {
                ...
                inputValues[j] = preFilterData( j, trainingUnits[i][0][j], preFiltLeakage );    // If preFiltLeakage is negative, no filtering is done
...
void EMGapproxESN::test( bool isValidation ) {
    ...
    // Set iteration limit based on operation mode
    unsigned int unitSize = isValidation ? validationUnits.size() : testingUnits.size();
    ...
    for( unsigned int i = 0; i < unitSize; i++ ) {
        ...
        for( unsigned int j = 0; j < numberOfInputs; j++ ) {
            ...
            if( isValidation ) {
                ...
                inputValues[j] = preFilterData( j, validationUnits[i][0][j], preFiltLeakage );    // If preFiltLeakage is negative, no filtering is done

            } else {

                inputValues[j] = preFilterData( j, testingUnits[i][0][j], preFiltLeakage );    // If preFiltLeakage is negative, no filtering is done
