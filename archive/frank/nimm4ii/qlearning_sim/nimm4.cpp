...
Nimm4::Nimm4(const OdeHandle& odeHandle, const OsgHandle& osgHandle,
		...
		bool sphereWheels /*=true*/, Nimm4Conf conf_ /*= getDefaultConf() */)
: // calling OdeRobots construtor with name of the actual robot
		OdeRobot(odeHandle, osgHandle, name, "$Id: nimm4.cpp,v 1.18 2010/03/29 07:11:53 martius Exp $")
{
	...
	//bcolor = Color(2, 156/255.0, 0, 1.0f);
	wcolor = Color(0.8, 0.8, 0.8, 1.0f); //wheels are white

	// set texture for body and wheels
	btexture = "Images/wood.rgb";
	wtexture = "Images/wood.rgb";
	...
	if (conf_.rpos_sensor_references.size()>0) {
		// Relative position sensor
...
int Nimm4::getSensors(sensor* sensors, int sensornumber){
	...
	int len = 0;
	// for each (joint) sensor the anglerate of the joint is red and scaled with 1/speed
	for (int i=0; i<4; i++){
		sensors[len]=joint[i]->getPosition2Rate();
		sensors[len]/=speed;  //scaling
		len++;
	}
	...
	if (rpos_sensing_active) {
		...
		//sensors[5]= sign(sensors[4]) * atan (sensors[5]/sensors[4]) * 180 / M_PI; // angle in degrees
	}
	// ---- relative position sensor ----
...
void Nimm4::create( const osg::Matrix& pose ){
	...
	for (int i=1; i<5; i++) {
		...
		sph->setTexture(wtexture);
		OsgHandle bosghandle = osgHandle;
		sph->init(wheelHandle, wmass, bosghandle.changeColor(wcolor));
	...
	if (rpos_sensing_active) {
		for (std::vector<RelativePositionSensor>::iterator it = rpos_sensor.begin(); it<rpos_sensor.end();it++){
			it->init(cap); // connect sensor to main body
		}
	}
