...
NeuralLocomotionControlAdaptiveClimbing::NeuralLocomotionControlAdaptiveClimbing() {
  ...
  TC_error.resize(AMOSII_MOTOR_MAX);
...
std::vector<double> NeuralLocomotionControlAdaptiveClimbing::step_nlc(const std::vector<double> inreflex,
   ...
   *******************************************************************************/
	Control_input = nlc->Control_input; //wave for climbing
	nlc->setCpgWeight(0, 1, 0.18 + Control_input);
	nlc->setCpgWeight(1, 0, -0.18 - Control_input);

  for (unsigned int i = 0; i < 5; i++) {
    nlc->setInputNeuronInput(i, input.at(i));
  }

  if((inreflex.at(115) - old_position)>0.0)
    sum_robot_speed += inreflex.at(115) - old_position;
  old_position = inreflex.at(115);
  average_robot_speed = sum_robot_speed/(10.0*global_count);

  //Turning
  input.at(3)=-1.0 + 4.0*(inreflex.at(119));
  input.at(4)=-1.0 - 4.0*(inreflex.at(119));
  if(input.at(3)> 1.0)
	  input.at(3)=1.0;
  if(input.at(3)< -1.0)
	  input.at(3)=-1.0;
  ...
  if(input.at(4)< -1.0)
	  input.at(4)=-1.0;
/*  cout << (inreflex.at(119)) << " " << input.at(3) << " " << input.at(4) << endl;*/
  ...
  if (switchon_backbonejoint && global_count > 50) {
    ...
    bjc->setInput(2, neg(bjc->getOutput(5)));
    bjc->setInput(3, pos(bjc->getOutput(5)));
    bjc->step();
...
/*  if (!switchon_backbonejoint) {
	...
	if(switchon_timer){
		bjc_timer++;
//	  cout << bjc_timer << " " << switchon_backbonejoint << endl;
	  if(bjc_timer>400){
		  	  switchon_backbonejoint = true;
		  	  switchon_timer = false;
		  	  bjc_timer=0;
...
//      << ' ' << m_pre.at(CL2_m) << ' ' << reflex_fs.at(L2_fs) << endl;
  ...
  for (unsigned int i = TR0_m; i < (BJ_m); i++) {
    ...
    if (fmodel.at(i)->error_neg > 6.5 || reflex_irs.at(motormap.at(i)->getIRsensor(i)) > 0.9 || TC_error.at(i) > 0.9 || reflex_fs.at(R0_fs + (i%6)) == -1.0) {
      if (delta_m_pre.at(i % 6) > 0.0 && switchon_irreflexes) {
        ...
        if (!motormap.at(i)->TC(i)) {
          m_reflex.at(i) = motormap.at(i)->getElevator(i);
/*          std::cout << i << " -> " << motormap.at(i)->getElevator(i) << endl;*/
        }
        m_deg.at(i) = motormap.at(i)->getDegrees(i, m_reflex.at(i));
      }
    }

//    if(average_robot_speed < 0.001 && (relative_position < 0.6 && relative_position > 0.4)){
//      if(i%6==1 ||i%6==4){
//        if (delta_m_pre.at(i % 6) > 0.0 && switchon_irreflexes) {
//          if (motormap.at(i)->TC(i) && in0.at(i) > 0.4) {
//            m_reflex.at(i) -= motormap.at(i)->getElevator(i);
...
*/
   ...
   *******************************************************************************/
   ...
   ac_motor = inreflex.at(A_cs); //raw data from ZAP 25 sensor
   ...
   incli_x = 131-inreflex.at(In_x);
   incli_y = 131-inreflex.at(In_y);

   //Convert inclinometer to degree:

   incli_x_deg = asin(incli_x*3.1416/180)*180/3.1416;
   incli_y_deg =  asin(incli_y*3.1416/180)*180/3.1416;

   //X = 131-Inclinometer input
   //Y = X*3.1416/180
   //Z = Asin(Y)

   //  Final degree = Z*180/3.1416;
   //
   //  + deg = tile to right
   //  - deg = til to left
...
/*   outFilenlc1<<ac_motor<<' '<<i_motor<<' '<<motor_power_con<<' '<<inreflex.at(In_x)<<' '<<inreflex.at(In_y)<<' '<<incli_x<<' '<<incli_y<<' '<<incli_x_deg<<' '<<incli_y_deg<<' '<<inreflex.at(R0_fs)<<' '<<inreflex.at(R1_fs)<<' '<<inreflex.at(R2_fs)<<' '<<inreflex.at(L0_fs)<<' '<<inreflex.at(L1_fs)<<' '<<inreflex.at(L2_fs)<<' '<<endl;*/
  ...
  //TC error TESTING (for elevator reflexes)
  for(int i=TR0_m;i < CR0_m;i++){
	  if(m.at(i + 6) > 0.8 && in0.at(i + 19) < -0.99){
		  TC_error.at(i) = 1.0;
	  }
	  else{
		  TC_error.at(i) = 0.0;
