    ...
    for( unsigned int r = 0; r < repetition; r++ ) {
        ...
        for( unsigned int i = 0; i < trainingUnits.size(); i++ ) {
            ...
            for( unsigned int j = 0; j < numberOfInputs; j++ ) {
                ...
                inputValues[j] = trainingUnits[i][0][j];                      // Call this if pre-filtering is unnecessary
//                inputValues[j] = preFilterData( j, trainingUnits[i][0][j] );    // Call this if pre-filtering is needed
