...
data = np.loadtxt("data/dataIMU.dat",unpack=False)
index = data[:,0]
aX = data[:,1]
aY = -data[:,2]
aZ = data[:,3]
...
data = np.loadtxt("data/velocities.dat", unpack = False)
indexV = data[:,0]
posX = data[:,1]
posY = data[:,2]
posZ = data[:,3]
...
# ax.plot(index,aY, label="aY", color  = "green")
# plt.plot(indexV, vYaw, label="vYaw", color = "blue")
plt.plot(indexV,vY, label="vY", color = "green")
plt.plot(indexV,vX, label="vX", color = "red")
