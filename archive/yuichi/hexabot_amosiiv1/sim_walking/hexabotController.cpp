    ...
    // ######## calculatin of the Phase Reset and inhibition ###############
	  for(int s =0 ; s < CPGNUM; s++){
		  // detect  whether it touched or not
            bool touchF = eIsLegContact(s); //(sDataSet.forceData[s] > 0.01);
...
//                if ( ph2 < 2.* M_PI  && ph2 > (2.* M_PI * conf.oscConf[s].dutyRate + (2.* M_PI * (1. - conf.oscConf[s].dutyRate)) * 1./2.) && (time - prvSTime[s]) > 7.){
//                    prvSTime[s] =  time;
...
bool HexabotController::eIsLegContact(int ch){
    if(ch >= 0 && ch < CPGNUM){
        return (sDataSet.forceData[ch] > 0.01);
    }else return false;
}
