...
//! Version Control
#define VERSION_REPEATMODULATORS "RepModulators_Ver 1.05_20140816"
//
//
// 20140816 Ver 1.05
...
// class to chage a initial vector to search basin
//  You can set the initial value by vector
class cChangeIniPhase_arbitally_RepMod:public itfRepeatModulator{
private:
    // reference for the parametervalue to be changed
    osg::Vec2d& refParam;
    std::vector<osg::Vec2d> iniPhaseVec;
    int currentNum;
public:
    //constructer
    cChangeIniPhase_arbitally_RepMod(osg::Vec2d& ref_iniPhase_, std::vector<osg::Vec2d> iniPhaseVec_)
        :itfRepeatModulator(), refParam(ref_iniPhase_)
    {
        iniPhaseVec = iniPhaseVec_;
        currentNum = 0;
    }

    //destructor
    ~cChangeIniPhase_arbitally_RepMod(){}

    // change configs
    void changeConfigs(int simNum_){
        // change parameters
        double v_x = iniPhaseVec.at(currentNum).x();
        double v_y = iniPhaseVec.at(currentNum).y();
    ...
    // isFinshed
    bool isFinished(){
        if(iniPhaseVec.size() <= currentNum) return true;
        return false;
    }
