...
void MirmorphSimulation::addCallback(lpzrobots::GlobalData &global, bool draw, bool pause, bool control){
    ...
    if(_bad_robot_conf && currentCycle==1){
        currentCycle = _repeat_number-5;
    }
    ...
    if(_repeat_experiment){
        if(_globalData->sim_step>=(_time_factor*60.0*100.0) || _reboot_sim || _bad_robot_conf || reached || (angle_x<0.5 && angle_x>-0.5) || not_moving){
            ...
            std::vector<double> w0 = _controller->ico_controller[0]->getWeights();
            std::vector<double> w1 = _controller->ico_controller[1]->getWeights();
            std::vector<double> w2 = _controller->ico_controller[2]->getWeights();

            partial_result[15] = w0[1];
            partial_result[16] = w1[1];
            partial_result[17] = w2[1];
...
//        _globalData->agents.erase(_globalData->agents.begin());
        ...
        if(currentCycle <= _repeat_number-5){
            createMirmorph(-1);
            _controller->learn = true;
        }
        else{
            createMirmorph(5-(_repeat_number-currentCycle));
            _controller->learn = false;
        }
...
void MirmorphSimulation::createMirmorph(int orientation){
    ...
    if(_size_test){
        ...
        fconf.mirconf.length = escale(0.965608, 0.8, 3.2);
        fconf.mirconf.width = escale(0.619093, 0.6, 2.4);
        fconf.mirconf.height = escale(1, 0.36, 1.44);
        for(uint i=0; i<4; i++){
            fconf.mirconf.radius[i] = escale(0, 0.2, 3*fconf.mirconf.length/8.0);
            fconf.mirconf.wheelthickness[i] = escale(0.56285, 0.1, 0.6);
            fconf.mirconf.wheelheights[i] = escale(0, 0, fconf.mirconf.height/2.0);
        }
    ...
    if(orientation==-1){
        int r = rand();
        random_or = (MAX_OR-MIN_OR)*((float)r/RAND_MAX)+MIN_OR;
    }
    else{
        random_or = (MAX_OR-MIN_OR)*orientation/4+MIN_OR;
    }
...
double MirmorphSimulation::getResults(int argc, char** argv){
    ...
    for(uint i=_repeat_number-5; i<_repeat_number; i++){
        if(_results[i][0]==-1){
            temp_fitness = 0.05;
        }
        else{
