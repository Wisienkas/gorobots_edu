 ...
 * EMGapproxESN.cpp
 *
 * Contains the EMGapproxESN class member function definitions
 *
 * Created on:          ?? ??, ????
 * Last Modified on:    April 25, 2017
 *
 * Author: Timon Tomas (Not sure)
 *
 * Extended by Kristoffer Honore
 * Small modifications
 * now generates a new file every repetition
 * now writes out all inputs, outputs and targets
 *
 */
...
void EMGapproxESN::activate() {
   ...
   // if( !learningFlag ) {
    ...
    for( unsigned int j = 0; j < numberOfOutputs; j++ ) {
        resultsFile << ESN->outputs->val(j, 0) << "\t";
    }
    for( unsigned int j = 0; j < numberOfOutputs; j++ ) {
        resultsFile << targetValues[j] << "\t";
    }
       resultsFile << squaredError << "\n" << flush;


    //resultsFile << input_ESN << "\t" << output_ESN << "\t" << target_ESN << "\t" << squaredError << "\n" << flush; //SAVE DATA

}

void EMGapproxESN::train( string resultPath) {
