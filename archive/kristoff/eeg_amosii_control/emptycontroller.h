...
public:
	EmptyController() : AbstractController("EmptyController", "$Id: tripodgait18dof.cpp,v 0.1 $")
	{
		t = 0;
		...
		//----------------Added Code_ RC network---------------------------//
		readdata(); // read input and target signals
		//*****************************************************//
		
	};
	
	//----------------------Added Code RC network-------------------------//
	void readdata()
	{
		std::cout << "Read Data" << std::endl;
		DelimitedFileReader::read( "data/inputData.txt", '\t', inputData, 0  );
		DelimitedFileReader::read( "data/targetData.txt", '\t', targetData, 0  );
	}
	
	EEGapproxESN * RCPrg = new EEGapproxESN();
	float result;
	float * signalPoint = new float[numberOfChannels];
	float * targetPoint = new float[numberOfChannels];
	int toggleMove = 1;
	int i = 0;
	float thresholdupper = 0.2;
	float thresholdlower = -0.2;
	bool toggleI = false;
	bool toggleT = false;
	...
	virtual void stepNoLearning(const sensor* x_, int number_sensors, motor* y_, int number_motors){
		...
		{
			signalPoint[j] = inputData[i][j];
			targetPoint[j] = targetData[i][j];
			//cout << "Data Read: " << inputData[0][0] << endl;
			//cout << "Data Read: " << inputData[1][0] << endl;
		}
		i++;
		
		result = RCPrg->run(signalPoint, targetPoint); // The output of the network that you can use to control your robot
		
		//----------------------Added Code RC network-------------------------//


		//std::cout << "result: " << result << " time i: " << i << std::endl;
		
		if( result < thresholdupper && result > thresholdlower )
		{
			thresholdupper = 0.2;
			thresholdlower = -0.2;
			toggleI = !toggleI;
		...
		//std::cout << "toggle: " << toggleMove << " target: " << targetPoint[0] << std::endl;
		//********************************************************//
		...
		//left front
		//y_[TL0_m] = static_cast<int>(targetPoint[0]);
		//y_[CL0_m] = static_cast<int>(targetPoint[0]);
		y_[FL0_m] = 0;
