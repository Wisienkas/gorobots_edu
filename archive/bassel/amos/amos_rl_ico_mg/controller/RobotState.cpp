...
double RobotState::get_state_array() {
...
bool RobotState::Final_goal_is_reached()
{
	return final_goal_checked;
}

bool RobotState::Is_end_of_phase()
{
...
void RobotState::reach_new_state() {
...
double RobotState::get_reward(double* distances) {
...
bool RobotState::reset_robot_state() {
	epoch_number++;
	///////////////////////////////
	/*if (epoch_number > 000) {
		std::ofstream myfile;
...
void RobotState::set_test_trial()
{
...
double RobotState::bind_exploration(double actor_out, double &exploration, bool ico_active)
{
...
int RobotState::get_target_number() {
	return current_goal;
}
//exploration function
double RobotState::get_exploration(double Vt, double gain)
{
...
//////////////////////////////////////////////ICO learning part//////////////////////////
void RobotState::check_landmark_lock(double min_dis, int min_index) {
	if (!ignore_landmark) {
		old_index = min_index;
		if (min_dis < 0.04)
...
int RobotState::get_nearest_signal_index(double* distances) {
	//for (int i=0; i<11; i++)
...
//ICO output
double RobotState::get_ICO_out(double signal_value, double* distances, int index)
{
	if (ignore_landmark)
			return 0;
...
bool RobotState::is_test_trial()
{
	return ((epoch_number % TEST_TRY) == 0);
}

bool RobotState::Is_reflex_active(double* distances, int index) {
	if (distances[index] < REFLEX_RANGE)
			return true;
	return false;
}
//ICO learning
double* RobotState::learn_ico(double angle, double old_angle, double* distances, int index) {
	//ico learning
	if (ignore_landmark)
			return ico_weights;
...
void RobotState::submit_step_number(int step) {
