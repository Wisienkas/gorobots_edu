...
NeuralLocomotionControlAdaptiveClimbing::NeuralLocomotionControlAdaptiveClimbing() {
  ...
  LegDamage_flag[LEG_LH] = true;//false;
  for (int i = 0; i < 6; i++) {
...
// 	      time = 0;
  ...
  //TC_middle
  //Adjust
  min_tc_m_nwalking_deg = -10;//-30; //deg ** MIN -60 deg
  max_tc_m_nwalking_deg = 40;//20; //10; //deg ** MAX +60 deg
  min_tc_m_nwalking = -1;
  max_tc_m_nwalking = 1;

  //TC_rear
  //Adjust
  min_tc_r_nwalking_deg = -40;//-60; //deg ** MIN -70 deg
  max_tc_r_nwalking_deg = 10;//-10; //deg ** MAX +70 deg
  min_tc_r_nwalking = -1;
  max_tc_r_nwalking = 1;
  ...
  if (new_loop_flag) { //if a new loop is start, choose a proper combination of the period for the leg
    ...
    if (learning) {
      ...
      do {
        ...
        } while ( (LegDamage_flag[legnum_random] == true) || (legnum_random == LEG_RF) || (legnum_random == LEG_LF) );
