 ...
 *      Author: poramate
 *      NeuralLocomotionControlAdaptiveClimbing_L1.cpp
 *
 *      NOT Work!!
 *
 *      With default tripod gait = Turn left fail in sim
 *
 *    ChaosCont.setPeriod(4); //RF
      ChaosCont.setPeriod_cli(6); //RM
      ChaosCont.setPeriod_cli1(4); //RH
      ChaosCont.setPeriod_cli2(4); //LF
      ChaosCont.setPeriod_cli3(4); //LM
      ChaosCont.setPeriod_cli4(4); //LH
 */
...
ChaoscontrAndPostproc ChaosCont(4, 4, 4, 4, 4, 4);
...
NeuralLocomotionControlAdaptiveClimbing::NeuralLocomotionControlAdaptiveClimbing() {
  ...
  //Control option:
   ...
   learned_gait = true;//if set to true = use learned gait, if set to false = use all period 4 (default tripod gait)

  //Save files
  outFilenlc1.open("TC-joint.txt");
  outFilenlcr.open("CTr-joint.txt");
  outFilenlft.open("FTi-joint.txt");
  recordperiod.open("trials.txt");
  ...
  buffer_c.resize(127);
  buffer_f.resize(127);
  buffer_tl.resize(127);
  buffer_cl.resize(127);
  buffer_fl.resize(127);

  buffer_t1.resize(127); //added by guanjiao
  buffer_c1.resize(127);
  buffer_f1.resize(127);
  buffer_tl1.resize(127);
  buffer_cl1.resize(127);
  buffer_fl1.resize(127);

  buffer_t2.resize(127);
  buffer_c2.resize(127);
  buffer_f2.resize(127);
  buffer_tl2.resize(127);
  buffer_cl2.resize(127);
  buffer_fl2.resize(127);
  ...
  LegDamage_flag[LEG_LF] = true; //false;
  LegDamage_flag[LEG_LM] = false;
  LegDamage_flag[LEG_LH] = false;
  for (int i = 0; i < 6; i++) {
    LegDamage_flag_tmp[i] = LegDamage_flag[i];
  }
...
// 	     deviation_distance_used = 3;
  ...
  //---define leg damage flag by Ren----
  for (unsigned int i = 0; i < 6; i++) { //initialize the default gait to period 4
    legperiod[i] = 4;
    legperiod_tmp[i] = 4;
    period_combination.push_back(legperiod[i]);
  }

  //--------period of leg and relevant values ---------
  period[P4] = 4;
  period[P5] = 5;
  period[P6] = 6;
  period[P8] = 8;
  period[P9] = 9;
  legnum_random = LEG_LF;
  periodnum_random = P9;
  //vectornum = 0;
  ChooseAnotherConbination = false;
  ...
  //---------decide learning or not--------------------
  Evaluation = 0;
  learning = true;
  keepnewperiod = true;

  //----------to output some experimental infomation------------
  time_t t = time(0);
  char tmp[64];
  strftime(tmp, sizeof(tmp), "%Y/%m/%d %X %A %j %z", localtime(&t));

  recordperiod << "Leg damage experiments" << endl;
  recordperiod << "Time: " << tmp << endl << endl;
  recordperiod << "Leg status: " << endl;
  recordperiod << "Leg RF: " << (!LegDamage_flag[LEG_RF] ? "working" : "fail") << endl;
  recordperiod << "Leg RM: " << (!LegDamage_flag[LEG_RM] ? "working" : "fail") << endl;
  recordperiod << "Leg RH: " << (!LegDamage_flag[LEG_RH] ? "working" : "fail") << endl;
  recordperiod << "Leg LF: " << (!LegDamage_flag[LEG_LF] ? "working" : "fail") << endl;
  recordperiod << "Leg LM: " << (!LegDamage_flag[LEG_LM] ? "working" : "fail") << endl;
  recordperiod << "Leg LH: " << (!LegDamage_flag[LEG_LH] ? "working" : "fail") << endl;
  recordperiod << endl << endl;
  recordperiod << "NO." << "\t" << "RF" << "\t" << "RM" << "\t" << "RH" << "\t" << "LF" << "\t" << "LM" << "\t" << "LH"
      << endl;
  ...
  offset_ftil_downward.resize(3); //KOH-->Eduard
   ...
   if(lift_body_up==true)
   {
     ...
     {
       int lifting_value = 3;//10;//5;//10; //20
...
std::vector<double> NeuralLocomotionControlAdaptiveClimbing::step_nlc(const std::vector<double> in0,
  ...
  if (0 == (global_count % TRIALSTEP) && (global_count > 100)) {
//    new_loop_flag = true;

    //deviation_angle_raw is used for avoiding the sharply ascending and descending of deviation_angle
    //When the robot turn cross the -180(180) degree, +(-) 360 degree;
    double deviation_angle_raw = in0[G0angleyaw_s] - ref_angle;
    if (deviation_angle_raw < -250)
      deviation_angle_raw += 360;
    if (deviation_angle_raw > 250)
      deviation_angle_raw -= 360;
    deviation_angle = ABS( deviation_angle_raw );

    if (deviation_angle < DEVI_THRESH) {
      learning = false; // if the robot is close to target, stop learning
    } else {
      ref_angle = in0[G0angleyaw_s];
    }

    //To output the period and deviation information
    if (!keepnewperiod)
      recordperiod << endl;
    recordperiod << trials++ << "\t";
    for (int i = 0; i < 6; i++) {
      recordperiod << legperiod[i] << "\t";
    }
    recordperiod << deviation_angle << endl;
  ...
  //starting!
  //learning mechanism
  if (new_loop_flag) { //if a new loop is start, choose a proper combination of the period for the leg
    ...
    if (Evaluation < 0) { //if the new deviation is less than last time, the new period is suitable for locomotion
      ...
      //remember the leg period for future use
      for (int i = 0; i < 6; i++) {
        legperiod_tmp[i] = legperiod[i];
      }
    } else {
      double y = exp(-BETA * Evaluation); // a evaluation number generated by the deviation , 0 ~ 1
      srand((unsigned) time(NULL));
      double x = (double) rand() / RAND_MAX; //a random value between 0 ~ 1
      if (x < y) { // if the evaluation number is bigger than a random value, which means the deviation decreased, keep the new period
        keepnewperiod = true;
        deviation_angle_used = deviation_angle;
        //remember the leg period for future use
        for (int i = 0; i < 6; i++) {
        ...
        keepnewperiod = false;
        for (int i = 0; i < 6; i++) { //return the leg period of last time
          legperiod[i] = legperiod_tmp[i];
        }
        cout << "return original period;" << endl;
      }
    }

    //To generate a random leg
    //cout<<"new loop begin"<<endl;
    if (learning) {
      int j = 0; // to count the number of generating random value
      do {
        ChooseAnotherConbination = false;
        ...
        do {
          ...
          legnum_random = rand() % 6;
        } while (LegDamage_flag[legnum_random] == true);

        //choose a random period
        srand((unsigned) time(NULL) - 5 + j);
        periodnum_random = rand() % 5;

        switch (legnum_random) {
          case LEG_RF:
            legperiod[LEG_RF] = period[periodnum_random];
            break;
          case LEG_RM:
            legperiod[LEG_RM] = period[periodnum_random];
            break;
          case LEG_RH:
            legperiod[LEG_RH] = period[periodnum_random];
            break;
          case LEG_LF:
            legperiod[LEG_LF] = period[periodnum_random];
            break;
          case LEG_LM:
            legperiod[LEG_LM] = period[periodnum_random];
            break;
          case LEG_LH:
            legperiod[LEG_LH] = period[periodnum_random];
            break;
          default:
        ...
        // check all the history values
        for (int i = 0; i < period_combination.size(); i = i + 6) {
          //if the current period conbination is the same as one history value, choose another period conbination
          if ((legperiod[LEG_RF] == period_combination.at(i)) && (legperiod[LEG_RM] == period_combination.at(i + 1))
              && (legperiod[LEG_RH] == period_combination.at(i + 2))
              && (legperiod[LEG_LF] == period_combination.at(i + 3))
              && (legperiod[LEG_LM] == period_combination.at(i + 4))
              && (legperiod[LEG_LH] == period_combination.at(i + 5))) {
            ChooseAnotherConbination = true;
            //return to original combination
            for (int i = 0; i < 6; i++) { //return the leg period of last time
              legperiod[i] = legperiod_tmp[i];
            }
            cout << j++ << " times same value  " << endl;
            break;
          }
        }

      } while (ChooseAnotherConbination);
      cout << "legnum_random = " << legnum_random << "  111111111111111111111111" << endl;
      cout << "periodnum_random = " << periodnum_random << "  2222222222222222222222" << endl;
      for (int i = 0; i < 6; i++) { //push the new combination to vector
        period_combination.push_back(legperiod[i]);
      }
    }
    ...
    ChaosCont.setPeriod_cli2(legperiod[LEG_LF]); //LF
    ChaosCont.setPeriod_cli3(legperiod[LEG_LM]); //LM
    ChaosCont.setPeriod_cli4(legperiod[LEG_LH]); //LH

    //--------- stop the leg and remember the current state ----------
    for (unsigned int i = 0; i < 6; i++) {
      LegDamage_flag_tmp[i] = LegDamage_flag[i];
      LegDamage_flag[i] = true;
    }
    step_tmp = global_count;
    //-----------------------------------------------------------------

    new_loop_flag = false;
  }

  if(learned_gait==true)
  {
    // ------------------ stop the robot at initial start -------------------
    if (global_count == 0) {
      //    for (unsigned int i = 0; i < 6; i++) {
      //      LegDamage_flag_tmp[i] = LegDamage_flag[i];
      //      LegDamage_flag[i] = true;
...
//		}
  ...
  if (reflex_R_fs.at(0) > 0 && reflex_R_fs.at(1) > 0 && reflex_R_fs.at(2) > 0 && reflex_L_fs.at(0) > 0
      && reflex_L_fs.at(1) > 0 && reflex_L_fs.at(2) > 0) {
...
//					a3_l.at(2)=-1.19105e-61
  ...
  for (unsigned int i = 0; i < acc_cmr_error.size(); i++) {
    ...
    m_reflex.at(i + CR0_m/*6*/) = (((m_pre.at(i + CR0_m/*6*/) - min_ctr) / (max_ctr - min_ctr))
        * (max_ctr_nwalking.at(i) - min_ctr_nwalking.at(i))) + min_ctr_nwalking.at(i);
    ...
    m_reflex.at(i + CL0_m/*9*/) = (((m_pre.at(i + CL0_m/*9*/) - min_ctr) / (max_ctr - min_ctr))
        * (max_ctl_nwalking.at(i) - min_ctl_nwalking.at(i))) + min_ctl_nwalking.at(i);
    //-----------Searching reflexes END-----------------------------------------//
  ...
  for (unsigned int i = 0; i < acc_cmr_error.size(); i++) {
    ...
    m_reflex.at(i + FR0_m/*12*/) = (((m_pre.at(i + FR0_m/*12*/) - min_fti) / (max_fti - min_fti))
        * (max_ftir_nwalking.at(i) - min_ftir_nwalking.at(i))) + min_ftir_nwalking.at(i);
    //-----------Searching reflexes END-----------------------------------------//
    ...
    m_reflex.at(i + FL0_m) = (((m_pre.at(i + FL0_m) - min_fti) / (max_fti - min_fti))
        * (max_ftil_nwalking.at(i) - min_ftil_nwalking.at(i))) + min_ftil_nwalking.at(i);
    //-----------Searching reflexes START----------------------------------------//
  ...
  } else // using normal control
  {
    ...
    if (true == LegDamage_flag[LEG_RF]) { //fix the leg
      m.at(TR0_m) = 0;
      m.at(CR0_m) = 0.7;
      m.at(FR0_m) = -0.7;
    }
    ...
    if (true == LegDamage_flag[LEG_RM]) { //fix the leg
      m.at(TR1_m) = 0;
      m.at(CR1_m) = 0.7;
      m.at(FR1_m) = -0.7;
    }
    ...
    if (true == LegDamage_flag[LEG_RH]) { //fix the leg
      m.at(TR2_m) = 0;
      m.at(CR2_m) = 0.7;
      m.at(FR2_m) = -0.7;
    }
    ...
    if (true == LegDamage_flag[LEG_LF]) { //fix the leg
      m.at(TL0_m) = 0;
      m.at(CL0_m) = 0.7;
      m.at(FL0_m) = -0.7;
    }
    ...
    if (true == LegDamage_flag[LEG_LM]) { //fix the leg
      m.at(TL1_m) = 0;
      m.at(CL1_m) = 0.7;
      m.at(FL1_m) = -0.7;
    }
    ...
    if (true == LegDamage_flag[LEG_LH]) { //fix the leg
      m.at(TL2_m) = 0;
      m.at(CL2_m) = 0.7;
      m.at(FL2_m) = -0.7;
    }
