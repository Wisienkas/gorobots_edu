...
*/
 	     ...
 	     {	//initialize the default gait to period 4
 	    	legperiod[i] = 4;
 	    	legperiod_tmp[i] = 4;
 	    ...
 	    recordperiod<<"NO."<<"\t"<<"RF"<<"\t"<<"RM"<<"\t"<<"RH"<<"\t"<<"LF"<<"\t"<<"LM"<<"\t"<<"LH"<<endl;
...
std::vector<double> NeuralLocomotionControlAdaptiveClimbing::step_nlc(const std::vector<double> in0, const std::vector<double> in1){//, bool Footinhibition){
	   ...
	   //Input to control locomotion
		...
		delta_devia_x  = ABS(Deviation_of_x - in0[G0x_s] );
		Deviation_of_x = in0[G0x_s];

		if( (delta_devia_x > 2.2) && (global_count>600) )
		{
			new_loop_flag = true;
			...
			if( !keepnewperiod ) recordperiod<<endl;
			recordperiod<<trials++<<"\t";
			for(int i = 0;i<6;i++)
			{
				recordperiod<<legperiod[i]<<"\t";
			}
			recordperiod<<deviation_distance<<endl;

		}
		distance  = sqrt( in0[G0y_s]*in0[G0y_s] + in0[G0x_s]*in0[G0x_s] );
		// --------------deviation signal by Ren--------------------
...
/*		cpg_w.at(0).at(0) =  1.4;
	...
	if(new_loop_flag)	//if a new loop is start, choose a proper combination of the period for the leg
	{
		...
		else
		{
			...
			if( x < y )
			{// if the evaluation number is bigger than a random value, which means the deviation decreased, keep the new period
				keepnewperiod = true;
				deviation_distance_used = deviation_distance;
				//remember the leg period for future use
				for(int i = 0; i< 6; i++)
		...
		if (learning)
		{
			...
			do
			{
				...
				{
					if( ( legperiod[LEG_RF] == period_combination.at(i) ) &&
						( legperiod[LEG_RM] == period_combination.at(i+1) ) &&
