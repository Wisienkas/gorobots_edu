...
: AbstractRobot("AmosIISerialV2", "$Id: main.cpp,v 0.1 2011/14/07 18:00:00 fhesse $") {
  ...
  //make sure your account in PC can have access to serial port
  // O_RDWR     : Open for reading and writing. The result is undefined if
  //              this flag is applied to a FIFO.
  // O_NOCTTY   : If set and path identifies a terminal device, open() will
  //              not cause the terminal device to become the controlling
  //              terminal for the process.
  // O_NONBLOCK : When opening a block special or character special file that
  //              supports non-blocking opens:
  //              If O_NONBLOCK is set:
  //                The open() function will return without blocking for the
  //                device to be ready or available. Subsequent behaviour of
  //                the device is device-specific.
  //              If O_NONBLOCK is clear:
  //                The open() function will block the calling thread until
  //                the device is ready or available before returning.
  fd1=open(port, O_RDWR | O_NOCTTY | O_NONBLOCK);

  if(fd1 == -1)
  {
    std::cout << "\nunable to open " << port << std::endl;
    std::cout << "check provided port "
                 "(hint: \"/dev/ttyS0\" or \"/dev/ttyUSB0\" required?)"
              <<std::endl;
    assert (fd1 != -1);
  }
  else
  {
    ...
    tio.c_oflag = 0;
    tio.c_cflag =
          CS8     // Character size: 8n1, see termios.h for more information
        | CREAD   // Enable receiver //
        | CLOCAL; // Ignore modem control lines.
    // local modes
    tio.c_lflag = 0;
    tio.c_cc[VMIN]=1;  // Minimum number of characters for noncanonical read
    tio.c_cc[VTIME]=5; // Timeout in deciseconds for noncanonical read
    ...
    std::cout << "port finish configuration"  << std::endl;
  }
...
int AmosIISerialV2::getSensors(sensor* sensors, int sensornumber){
  ...
  msg[0] = comByte;
  msg[1] = end;
  if (write(fd1, msg,2)!=2)
    std::cerr<<"not all bytes written in getSensors" << std::endl;

  // --- Reading the potentiometer values
  double potValue[SENSOR_BUFFER_NUM];
  for (int i=1;i<SENSOR_BUFFER_NUM;i++){
    char chBuff;
    //std::cout<< "-startrd-" << std::endl;
    while (read(fd1, &chBuff, 1)==0);
    //std::cout<< "-endrd-" << std::endl;
    // potvalue are AMOS sensor data
    potValue[i]=(unsigned char)(chBuff);
  }
  ...
  // angle sensors
  sensors[TR0_as]=potValue[TR0_as_real];
...
void AmosIISerialV2::processSensors(sensor* psensors){
	...
	// angle sensor (Timo)
	psensors[TR0_as]=(psensors[TR0_as]-88.0)/50.0;
...
void AmosIISerialV2::setMotors(const motor* motors, int motornumber){
  ...
  //Setting all motor to middle position as initialization
  std::vector<int> serialPos(MOTOR_BUFFER_NUM, 128);
  ...
  const char end =0; // Null as Sync-byte
  const char comByte=1;

  char msg[34]={};
  msg[0] = comByte;
  for (int i=1;i<33;i++) msg[i] = serialPos[i];
  msg[33] = end;

  //Sendding command to serial port
  if (write(fd1, msg, 34) != 34)
    std::cerr << "not all bytes written in setMotors" << std::endl;
