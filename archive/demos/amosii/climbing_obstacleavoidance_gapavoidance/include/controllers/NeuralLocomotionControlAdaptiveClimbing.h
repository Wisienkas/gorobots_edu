...
#ifdef LASERSCANNER
    int countClimbing; //Counter for climbing
    int countAvoidance; //Counter for avoidance
    int countRough; //Counter for change of walking pattern

    bool log; //Logging or not
    ofstream out; //logging stream

    //Parameters for decision making
	double heightThreshold; //Height Threshold for roboter in mm
	double gapThreshold; //Maximal depth of gap which is still traversable by the robot
	double roughThreshold; //Terrain to rough. Change walking pattern
	double rough2Threshold; //Very rough terrain
	double groundThreshold; //Below is considered noise
	double distThreshold; //Distance at which to start climbing (sensor values of IR sensor)
	double walkingPatternRough; //Control input for walking pattern on rough surface

	int climbingThreshold; //Steps after which climbing is stopped
	int avoidanceThreshold; //Steps after which avoidance is stopped
	int roughStepThreshold; //Steps after which walking pattern is changed when detecting rough terrain

	double oldWalkingPattern; //Initial control input of the robot
#endif
    //------------------------------------------------
