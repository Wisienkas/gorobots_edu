...
nMotors =  motornumber;
steps = 0;
SigmoidTransitionFunction transF = SigmoidTransitionFunction(300);
MuscleModelConfiguration config = MuscleModelConfiguration(transF);
config.D = 0.1;         		//0.2
config.Dmin = 1;	  		//0.2
config.mass = simulatedMass;//1.5
config.radius = 0.6; 		//1
config.mActFactor = 1.9;	//1.2
config.K = 1;
config.Kmin = 0.4;	  		//0.2
config.centerAngle = M_PI*0.9; //0.9
config.groundAngle = M_PI;
config.length = 0.122;
config.timestep = 4;
LKmuscles = new DCControllingVAAMs(config);
addParameterDef("LKK",&(LKmuscles->K),0.8,0.0,8.0,"Stiffness of the left knee [0 .. 10.0]");
addParameterDef("LKD",&(LKmuscles->D),0.01,0.0,2,"Damping of the left knee [0 .. 10.0]");
RKmuscles = new DCControllingVAAMs(config);
addParameterDef("RKK",&(RKmuscles->K),0.8,0.0,8,"Stiffness of the right knee [0 .. 10.0]");
addParameterDef("RKD",&(RKmuscles->D),0.01,0.0,2,"Damping of the right knee [0 .. 10.0]");
config.centerAngle = M_PI/2.1;
config.K = 2.0;
config.D = 0.15;
config.groundAngle = M_PI/2.1;
config.length = 0.142;
config.radius = 0.5;
config.mActFactor = 2; //1.5
LHmuscles = new DCControllingVAAMs(config);
addParameterDef("LHK",&(LHmuscles->K),0.5,0.0,8,"Stiffness of the left hip [0 .. 1.0]");
addParameterDef("LHD",&(LHmuscles->D),0.01,0.0,2,"Damping of the left hip [0 .. 10.0]");
RHmuscles = new DCControllingVAAMs(config);
addParameterDef("RHK",&(RHmuscles->K),0.5,0.0,8,"Stiffness of the right hip [0 .. 1.0]");
addParameterDef("RHD",&(RHmuscles->D),0.01,0.0,2,"Damping of the right hip [0 .. 10.0]");

leftMuscles = new MuscleChain(2);
leftMuscles->addMuscle(1,LKmuscles);
leftMuscles->addMuscle(0,LHmuscles);

rightMuscles = new MuscleChain(2);
rightMuscles->addMuscle(1,RKmuscles);
rightMuscles->addMuscle(0,RHmuscles);
...
void MuscleRunbotController::step(const sensor* sensors, int sensornumber, motor* motors, int motornumber) {
  ...
  leftMuscles->setState(1,motorOutput[2],leftLoad,lKAng);
  rightMuscles->setState(1,motorOutput[3],rightLoad,rKAng);
  leftMuscles->setState(0,motorOutput[0],leftLoad,lHAng);
  rightMuscles->setState(0,motorOutput[1],rightLoad,rHAng);

  double tFactor = 1.05; //torquefactor, to  scale muscle models output into the right range (to motor voltage)

  /**
   * second step for muscle chain: get all new output values!
   */
  LKMusclTor = tFactor*leftMuscles->getSignal(1);
  RKMusclTor = tFactor*rightMuscles->getSignal(1);
  LHMusclTor = tFactor*leftMuscles->getSignal(0);
  RHMusclTor = tFactor*rightMuscles->getSignal(0);
