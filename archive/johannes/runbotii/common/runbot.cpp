...
namespace lpzrobots {
  ...
  void Runbot::create(const Matrix& pose){
    ...
    double shoulderHeight = tempHeight+conf.bodyHipHeight*s;
    ...
    if (conf.armLength > 0){
      ...
      arm->init(odeHandle,0.05,osgHandle);
  ...
  void Runbot::setMotorsIntern(const motor* motors, int motorNumber){
    ...
    if (conf.invertMotors)
      inv = -1;
    double inputFactor = 1.0/2.4; //2.4 because the voltage is doubled in the amplifier box on the real robot, so a control voltage of 2.4 refers to 4.8V
  ...
  void Runbot::doInternalStuff(GlobalData& globalData){
    ...
    //ankle springs
    passiveMotors[0]->set(0.0);
    passiveMotors[1]->set(0.0);
