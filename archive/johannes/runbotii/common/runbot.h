  ...
  class Runbot: public OdeRobot{
    public:
      ...
      static RunbotConf getDefaultConf(){
        ...
        double torquefactor = 0.42; //0.8
        //---------------Motor parameters------------------//
        //Todo: need good, realistic values!
        conf.kneeMotorPower = 3.0*torquefactor;
        conf.kneeMotorMin = -M_PI*0.5;
        conf.kneeMotorMax = 0.0;
        conf.kneeMotorMaxVel = 10.54*speedfactor; //rad / sec

        conf.hipMotorPower = 5.5*torquefactor;
        conf.hipMotorMin = -0.2*M_PI;//0.17
        conf.hipMotorMax = +0.2*M_PI;//0.17
        conf.hipMotorMaxVel = 5.82*speedfactor; //rad / sec // should be 5.82, but then knees are too slow
        ...
        //passive Motors
        conf.ankleMotorPower =0.5*torquefactor;
