...
double PositionControllingVMM::getOutput(double muscleAct, double exForce,
	...
	double x1,v1,a1,x2,v2,a2,x3,v3,a3,x4,v4,a4,xf,vf;
	               ...
	               x1 = preAngle;
	               v1 = preSpeed;
	               a1 = fM->getTorque(x1,x1+zeroAngle-groundAngle, v1, timeSteps, preExforce, preMusclAct, preDisVec, K, D, length, mActFactor);

	               x2 = preAngle + (0.5 * v1 * timeSteps);
	               v2 = preSpeed + (0.5 * a1 * timeSteps);
	               a2 = fM->getTorque(x2,x2+zeroAngle-groundAngle, v2, timeSteps/2.0, (preExforce + exForce)/2.0, (preMusclAct + muscleAct)/2.0, (preDisVec + DisVec)/2.0, K, D, length, mActFactor);

	               x3 = preAngle + (0.5 * v2 * timeSteps);
	               v3 = preSpeed + (0.5 * a2 * timeSteps);
	               a3 = fM->getTorque(x3,x3+zeroAngle-groundAngle, v3, timeSteps/2.0, (preExforce + exForce)/2.0, (preMusclAct + muscleAct)/2.0, (preDisVec + DisVec)/2.0, K, D, length, mActFactor);

	               x4 = preAngle + (v3 * timeSteps);
	               v4 = preSpeed + (a3 * timeSteps);
	               a4 = fM->getTorque(x4,x4+zeroAngle-groundAngle, v4, timeSteps, exForce, muscleAct, DisVec, K, D, length, mActFactor);
