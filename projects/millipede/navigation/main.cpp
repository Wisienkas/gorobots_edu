...
class navigationSim : public lpzrobots::Simulation {
  public:
...
//    global.obstacles.push_back(playground);
  ...
  virtual bool restart(const OdeHandle&, const OsgHandle&, GlobalData& global)
  {
      ...
      if (count >= runs_x){
          sfeed= initial_sfeed;
          simN++;
          trWeight+=trWeight_step;
          count = 0;
      }


      std::cout << "run: " << count+simN*runs_x << std::endl << "S. feedback:\t" << sfeed << std::endl << "Followed Str.:\t" << followed << std::endl;
...
protected:
  ...
  NavigationCPGSF* controller;
  lpzrobots::Millipede* millipede;
  int number_of_runs, runs_x, count;
  double followed, S, sfeed, trWeight;
  double followed_step, S_step, sfeed_step, trWeight_step;
