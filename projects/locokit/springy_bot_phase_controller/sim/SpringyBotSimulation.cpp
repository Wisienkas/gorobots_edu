...
void SpringyBotSimulation::start(const OdeHandle& odeHandle, const OsgHandle& osgHandle, GlobalData& global) {
	...
	std::ofstream out;
	out.open("output.csv");
	out << "speed_sp, phase_sp, sim_time, cycle_time, max_distance, rear_max_time, tilt_min_time, tilt_max_time, tiltMin, tiltMax" << std::endl;// << "phaseSteps, 8, velSteps, 8" << std::endl;
...
void SpringyBotSimulation::addCallback(GlobalData& globalData, bool draw, bool pause, bool control) {
	...
	controller->setPositionData( robot->getPosition() );
}
...
void SpringyBotSimulation::instantiateAgent(GlobalData& global) {
	...
	controller = new SpringyBotPhaseController("LocoKit Controller", trial_number++);
