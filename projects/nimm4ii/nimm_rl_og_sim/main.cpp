...
#include <NimmOG.h> // ICO + AC 3 goals no obstacle
...
public:
	...
	void generate_spheres(GlobalData& global)
	{
		...
		for (int i=0; i < number_spheres; i++){
			...
			if (i==0) s1->setPosition(osg::Vec3(-2.0 /*position_S*/, position_S-13, 2/*0*/));
			//Target 2

			//if (i==1) s1->setPosition(osg::Vec3(15-11.0 /*position_S*/, position_S+10, 0.5/*0*/));
			if (i==1) s1->setPosition(osg::Vec3(20.0 /*position_S*/, position_S+7, 2/*0*/)); //Green Sphere

			//Target 3./s
			//if (i==2) s1->setPosition(osg::Vec3(15-11.0 /*position_S*/, position_S-10, 0.5/*0*/));
			if (i==2) s1->setPosition(osg::Vec3(5.0 /*position_S*/, position_S-18, 2/*0*/)); //Blue sphere

			//Target 4
			if (i==3) s1->setPosition(osg::Vec3(-10.0 /*position_S*/, position_S+7, 2/*0*/));
	...
	void setup_Playground(GlobalData& global)
	{
		...
		double length_pg = 25;//28;//0.0; //45, 32, 22
	...
	void start(const OdeHandle& odeHandle, const OsgHandle& osgHandle, GlobalData& global)
	{
		...
		setCameraHomePos(Pos(-1.14383, 10.1945, 142.7865),  Pos(179.991, -77.6244, 0));
		//1) - set noise to 0.1
