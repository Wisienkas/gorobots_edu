...
#include <NimmExp2.h> // ICO + AC 3 goals no obstacle
...
int number_spheres = 3; // SET NUMBER of TARGET MAX = 4
int number_boxes = 0; // SET NUMBER of BOXES obstacles
...
public:
	...
	void generate_spheres(GlobalData& global)
	{
		...
		for (int i=0; i < number_spheres; i++){
			...
			if (i==0) s1->setPosition(osg::Vec3(-2.0 /*position_S*/, position_S-11, 3/*0*/));
			//Target 2

			//if (i==1) s1->setPosition(osg::Vec3(15-11.0 /*position_S*/, position_S+10, 0.5/*0*/));
			if (i==2) s1->setPosition(osg::Vec3(- 2.0 /*position_S*/, position_S+7, 3/*0*/)); //Green Sphere

			//Target 3./s
			//if (i==2) s1->setPosition(osg::Vec3(15-11.0 /*position_S*/, position_S-10, 0.5/*0*/));
			if (i==1) s1->setPosition(osg::Vec3(10.0 /*position_S*/, position_S - 2 , 3/*0*/)); //Blue sphere

			//Target 4
			if (i==3) s1->setPosition(osg::Vec3(-10.0 /*position_S*/, position_S, 3/*0*/));
	...
	void generate_box(GlobalData& global)
	{
		...
		// - add sphere to list of obstacles
		PassiveBox* b1;
		double length = 13.5 /*for learning*/; //3.5 /*for*/;//testing//---------------------------------------------------------------------------TEST
		double width = 1.0;
		double height = 1.0;
		//if(obstacle_on)
		{
			for (int i=0; i < number_boxes /*SET NUMBER OF OBSTACLES*/; i++){
				...
				b1->setPose(osg::Matrix::rotate(/*-0.5*(M_PI/4)*/ (M_PI/2), 0,0, 1) * osg::Matrix::translate(/*i*4.5+*/pos_obstacle_x, pos_obstacle_y+i*-pos_obstacle_y*2.0-12/*0+i*/,height/2) /* pose*/);
				//obst.push_back(b1);
	...
	void start(const OdeHandle& odeHandle, const OsgHandle& osgHandle, GlobalData& global)
	{
		...
		setCameraHomePos(Pos(-1.14383, 10.1945, 102.7865),  Pos(179.991, -77.6244, 0));
		//1) - set noise to 0.1
		...
		if (ac_ico_robot)
		{
			...
			else
			{
				...
				/************************************/
				Pos pos(position_x-15.0/*, +x = to left, -x = to right*/,-21.0/*y*/,0.0/*z*/);
