 ...
 ***************************************************************************/
/*
4, template ashigaru4
 this is the program for straight connected ashigaru
 I applied phase reset plus inhibitation method
 and in addition to this I applied Koh method which make the difference between stancce phase and swing phase
*/
...
public:
  ...
  void start(const OdeHandle& odeHandle, const OsgHandle& osgHandle, GlobalData& global) 
  {
    ...
    for(int i = 0; i< Num; i++){
    	...
    	// make controller
        TestController4Conf confy;
        confy = TestController4::getDefaultConf();
        if( 0){
        	confy.disLeg = true;
        }else{
        	confy.disLeg = false;
        ...
        controller[i] = new TestController4(ashName2, 0.1, confy);//  TestController::TestController(confy);
    ...
    confy = TestController4::getDefaultConf();
    confy.disLeg = false;
    controller = new TestController4("data1", 0.1, confy);//  TestController::TestController(confy);
    controller->setParam("eps",0.1);
    controller->setParam("factor_a",0.01);

    confy.disLeg = true;
    controller2 = new TestController4("data2", 0.1, confy);//  TestController::TestController(confy);
    controller2->setParam("eps",0.1);
    controller2->setParam("factor_a",0.01);

    confy.disLeg = false;
    controller3 = new TestController4("data3", 0.1, confy);//  TestController::TestController(confy);
