...
#include "testController3.h"

using namespace matrix;
using namespace std;
using namespace ASHIGARU;

TestController3::TestController3(std::string name, double ini, const TestController3Conf& _conf)
: AbstractController("TestController3", "$Id: testController3.cpp,v 0.1 $"),
  conf(_conf) {
  t = 0;
  ...
  // prepare name;
//  Configurable::insertCVSInfo(name, "$RCSfile: TestController3.cpp,v $",
//      "$Revision: 0.1 $");
}
...
void TestController3::init(int sensornumber, int motornumber, RandGen* randGen)
{
...
/// inputs.
void TestController3::step(const sensor* x_, int number_sensors,
...
/// inputs.
void TestController3::stepNoLearning(const sensor* x_, int number_sensors,
    motor* y_, int number_motors) {
  ...
  */
  ...
  if(mode == 0){
	  ...
	  double cWeight = 0.8;//1.5
	  double tWeight = 0.8;//3.;
	  double fWeight = 3.;
	  double off3 = 0.3;//0.7
	  double off = -0.1;
	  double off2 = -0.35;
	  ...
	  if(cnctLeg == 0){
		  ...
		  // stopped Leg
		  y_[T1_m] = 0.;//outputH2 * conf.fact + conf.bias;
		  y_[C1_m] =  1.57; //outputH * conf.fact;//outputH2 * conf.fact;
		  y_[F1_m] =  -1.57;//- cWeight * outputH2 * conf.fact * dir * dir2 ;//outputH2 * conf.fact + conf.bias;

		  // moving leg 0
		  y_[T0_m] =  0.;//- tWeight * outputH1[1] * conf.fact * dir * dir2 + off3;// 0.3;
		  y_[C0_m] =  - cWeight * outputH2[1] * conf.fact * dir + off;
		  y_[F0_m] = -tWeight * outputH1[1] * conf.fact * dir * dir2 + off3;//outputH1 * conf.fact * conf.direction;
		  ...
		  // stopped Leg
		  y_[T2_m] = 0.;//outputH2 * conf.fact + conf.bias;
		  y_[C2_m] =  1.57; //outputH * conf.fact;//outputH2 * conf.fact;
		  y_[F2_m] =  -1.57;//- cWeight * outputH2 * conf.fact * dir * dir2 ;//outputH2 * conf.fact + conf.bias;
		  ...
		  // moving leg 1
		  y_[T1_m] =  -tWeight * outputH1[2] * conf.fact * dir * dir2 + off3;// - 0.3;
		  y_[C1_m] =  - cWeight *  outputH2[2] * conf.fact * dir + off;
		  y_[F1_m] = 0.;//-outputH1 * conf.fact * conf.direction;
	  }
...
bool TestController3::store(FILE* f) const {
  return true;
}

/** loads the controller values from a given file. */
bool TestController3::restore(FILE* f) {
  return true;
}

/** get default configulation **/
TestController3Conf TestController3::getDefaultConf() {
  TestController3Conf c;
  double a = 1.6;
  double phi = M_PI * 30./180.;
