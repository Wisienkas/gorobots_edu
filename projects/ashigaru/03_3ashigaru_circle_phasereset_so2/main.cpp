...
/*
3, template ashigaru3
 rhis is the program for circle connected ashigaru
 I applied phase reset plus inhibitation method
*/
...
//	Ashigaru2* ashiUnit4;
	...
	TestController3* controller[Num];
...
//	AbstractController* controller4;
	...
	lpzrobots::Joint* robotFixator[ Num ];
...
public:
  ...
  void start(const OdeHandle& odeHandle, const OsgHandle& osgHandle, GlobalData& global) 
  {
    ...
    for(int i = 0; i< Num; i++){
    	...
    	// make controller
        TestController3Conf confy;
        confy = TestController3::getDefaultConf();
        if(0){
        	confy.disLeg = true;
        }else{
        	confy.disLeg = false;
        }
        controller[i] = new TestController3("data1", 0.1, confy);//  TestController::TestController(confy);
        ...
        if(i < 2){
        	ashiUnit[i] = new Ashigaru(odeHandle, osgHandle, conf, "Ashigaru");
        	...
        	controller[i]->setCnctLegNumber(i);
        }else if(i == 2){
        	ashiUnit[i] = new Ashigaru(odeHandle, osgHandle, conf, "Ashigaru");
			ashiUnit[i]->setColor(Color(1.0,1.0,0));
			ashiUnit[i]->place( osg::Matrix::rotate(0., osg::Vec3(1, 0, 0)) * osg::Matrix::translate( conf.rate * conf.connectLength * cos(PI / 3.), conf.rate * conf.connectLength * sin(PI / 3.), 0.2));
			controller[i]->setCnctLegNumber(i);
        }
        // make agent
        ...
        if(i == 0){
        	...
        	// Create ASHIGARU connection
        	robotFixator[i-1] = new lpzrobots::FixedJoint(
        			ashiUnit[i-1]->getTibiaPrimitive(Ashigaru::L1),
        			ashiUnit[i]->getMainPrimitive());
            	//global.environment);
        	robotFixator[i-1]->init(odeHandle, osgHandle, false);

        	// Create ASHIGARU connection
        	robotFixator[2] = new lpzrobots::FixedJoint(
        			ashiUnit[2]->getTibiaPrimitive(Ashigaru::L2),
        			ashiUnit[0]->getMainPrimitive());
            	//global.environment);
        	robotFixator[2]->init(odeHandle, osgHandle, false);
        }
    ...
    confy = TestController3::getDefaultConf();
    confy.disLeg = false;
    controller = new TestController3("data1", 0.1, confy);//  TestController::TestController(confy);
    controller->setParam("eps",0.1);
    controller->setParam("factor_a",0.01);

    confy.disLeg = true;
    controller2 = new TestController3("data2", 0.1, confy);//  TestController::TestController(confy);
    controller2->setParam("eps",0.1);
    controller2->setParam("factor_a",0.01);

    confy.disLeg = false;
    controller3 = new TestController3("data3", 0.1, confy);//  TestController::TestController(confy);
