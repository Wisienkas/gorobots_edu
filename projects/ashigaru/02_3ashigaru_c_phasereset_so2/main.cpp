...
public:
  ...
  void start(const OdeHandle& odeHandle, const OsgHandle& osgHandle, GlobalData& global) 
  {
    ...
    for(int i = 0; i< Num; i++){
    	...
    	// make controller
        TestController2Conf confy;
        confy = TestController2::getDefaultConf();
        if( 0){
        	confy.disLeg = true;
        }else{
        	confy.disLeg = false;
        }
        controller[i] = new TestController2("data1", 0.1, confy);//  TestController::TestController(confy);
    ...
    confy = TestController2::getDefaultConf();
    confy.disLeg = false;
    controller = new TestController2("data1", 0.1, confy);//  TestController::TestController(confy);
    controller->setParam("eps",0.1);
    controller->setParam("factor_a",0.01);

    confy.disLeg = true;
    controller2 = new TestController2("data2", 0.1, confy);//  TestController::TestController(confy);
    controller2->setParam("eps",0.1);
    controller2->setParam("factor_a",0.01);

    confy.disLeg = false;
    controller3 = new TestController2("data3", 0.1, confy);//  TestController::TestController(confy);
