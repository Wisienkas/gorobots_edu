...
float X = 1.5, Y = 0, X_dot = 0, Y_dot = 0;
float phase = 0;
...
class EmptyController : public AbstractController {
  ...
  public:
    ...
    virtual void stepNoLearning(const sensor* x_, int number_sensors,
      ...
      const float T_fac = 0.1;
      
      const float C_fac = 0.25;
      float T_right = T_fac * 0;
      float T_left = T_fac * 2;
      
      int status = 1; // -1: left 1: right 0: forward, -2 backward
