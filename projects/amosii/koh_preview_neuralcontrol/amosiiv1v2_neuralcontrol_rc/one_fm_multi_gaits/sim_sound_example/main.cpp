  ...
  public:
    ...
    virtual void start(const lpzrobots::OdeHandle& odeHandle,
      ...
      if (use_amosii_version1 && !use_amosii_version2){
        ...
        controller = new SineController();// new AmosIIControl(/*amos version1 */);//TripodGait18DOF();
      ...
      if (use_amosii_version2 && !use_amosii_version1){
        ...
        amos->addSensor(std::make_shared<lpzrobots::SoundSensor>(lpzrobots::Sensor::X),
                        lpzrobots::Attachment(-1));


        // put amos a little bit in the air
        amos->place(osg::Matrix::translate(.0, .0, 0.5));

        controller = new SineController();// new AmosIIControl(/*amos version 2*/);//TripodGait18DOF();
