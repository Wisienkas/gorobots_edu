...
int main(int argc, char** argv){
  ...
  bool use_amosii_version1 = true;//false;
  bool use_amosii_version2 = false;//true;

  GlobalData globaldata;

  //if (use_amosii_version1 && !use_amosii_version2)
  AmosIISerialV1* robot1;

  //if (use_amosii_version2 && !use_amosii_version1)
  AmosIISerialV2* robot2;
  ...
  if (use_amosii_version1 && !use_amosii_version2){
    ...
    robot1         = new AmosIISerialV1("/dev/ttyS0");     // using serial port
  ...
  if (use_amosii_version2 && !use_amosii_version1){
    ...
    robot2        = new AmosIISerialV2("/dev/ttyS0");     // using serial port
    //robot         = new AmosIISerialV2("/dev/ttyUSB0");   // using USB-to-serial adapter
  ...
  if (use_amosii_version2 && !use_amosii_version1)
  agent->init(controller, robot2, wiring);
  ...
  if (use_amosii_version1 && !use_amosii_version2)
  globaldata.configs.push_back(robot1);
  if (use_amosii_version2 && !use_amosii_version1)
  globaldata.configs.push_back(robot2);
