...
#include <AmosMG.h> // ICO + AC 3 goals no obstacle
#include "amosII.h"
...
#define MIN_x   5.0  //5.0 (TEST) //Min range-------------------------------------------------CH VERY IMPORTANT FACTOR
...
int number_spheres = 8;//2; // SET NUMBER of TARGET MAX = 4
int number_boxes = 4; // SET NUMBER of BOXES obstacles
...
public:
	...
	void generate_spheres(GlobalData& global)
	{
		...
		for (int i=0; i < number_spheres; i++){
			s1 = new PassiveSphere(odeHandle, osgHandle, 0.1);
			//Target 1
			//position_S = 0.0;// -2.0---------------------------------------------------------------------------TEST
			if (i==0) s1->setPosition(osg::Vec3(5.0/*position_S*/, position_S - 3.7, 0.35/*0*/));
			if (i==1) s1->setPosition(osg::Vec3(3.0 /*position_S*/, position_S  - 4.7 , 0.35/*0*/));
			if (i==2) s1->setPosition(osg::Vec3(0.5/*position_S*/, position_S - 3, 0.35/*0*/));

			if (i==3) s1->setPosition(osg::Vec3(1.5/*position_S*/, position_S - 0.5 , 0.35/*0*/));
			//if (i==4) s1->setPosition(osg::Vec3(-1.5 /*position_S*/, position_S  - 1.5 , 2/*0*/));

			//if (i==5) s1->setPosition(osg::Vec3(0.0/*position_S*/, position_S  , 0.25/*0*/));
			//if (i==6) s1->setPosition(osg::Vec3(2.5/*position_S*/, position_S  , 0.25/*0*/));
			////////////////////////
			if (i==5) s1->setPosition(osg::Vec3(8.0 /*position_S*/, position_S - 7, 0.35/*0*/));
			if (i==6) s1->setPosition(osg::Vec3(- 1.0 /*position_S*/, position_S - 7, 0.35/*0*/));
			if (i==7) s1->setPosition(osg::Vec3(3.5 /*position_S*/, position_S + 2, 0.35/*0*/));

			s1->setTexture("Images/dusty.rgb");
			//Target 1
			if (i < 4){ s1->setColor(Color(1,0,0)); }
			else if (i == 4) {
				delete s1;
				s1 = new PassiveSphere(odeHandle, osgHandle, 0.2);
				s1->setPosition(osg::Vec3(4.3/*position_S*/, position_S - 0.5 , 0.5/*0*/));
				s1->setColor(Color(0,0,0));
			}
			else if (i == 7) { s1->setColor(Color(0,1,0)); }
			else { s1->setColor(Color(1,1,1)); }
			obst.push_back(s1);
	...
	void generate_box(GlobalData& global)
	{
		...
		PassiveBox* b1;
		double length = 12.5/4.0 /*for learning*/; //3.5 /*for*/;//testing//---------------------------------------------------------------------------TEST
		double width = 0.4;
		double height = 0.5;
		//if(obstacle_on)
		{
			for (int i=0; i < number_boxes /*SET NUMBER OF OBSTACLES*/; i++){
				if (i == 0) {
					b1 = new PassiveBox(odeHandle, osgHandle, osg::Vec3(length - 1, width, height /*size*/));
					b1->setColor(Color(0.1,0.1,0.5));
					b1->setPose(osg::Matrix::rotate((M_PI / 3), 0,0, 1) *
					osg::Matrix::translate(pos_obstacle_x - 0.7, pos_obstacle_y+i*-pos_obstacle_y*2.0-8.5,height/2));
				}
				else if (i == 1) {
					b1 = new PassiveBox(odeHandle, osgHandle, osg::Vec3(length , width, height /*size*/));
					b1->setColor(Color(0.1,0.1,0.5));
					b1->setPose(osg::Matrix::rotate((0), 0,0, 1) *
					osg::Matrix::translate(pos_obstacle_x + 1 , pos_obstacle_y+i*-pos_obstacle_y*2.0 + 0.5, height/2));
				}
				else if (i == 2) {
					b1 = new PassiveBox(odeHandle, osgHandle, osg::Vec3(length - 1, length - 1, height /*size*/));
					b1->setColor(Color(0.1,0.1,0.5));
					b1->setPose(osg::Matrix::rotate((M_PI / 4), 0,0, 1) *
					osg::Matrix::translate(pos_obstacle_x - 1.75, pos_obstacle_y+-pos_obstacle_y*2.0 + 0.5, height/2));
				}
				else if (i == 3) {
					b1 = new PassiveBox(odeHandle, osgHandle, osg::Vec3(15.5/4.0 ,5/4.0 , height /*size*/));
					b1->setColor(Color(0.1,0.1,0.5));
					b1->setPose(osg::Matrix::rotate((M_PI), 0,0, 1) *
					osg::Matrix::translate(pos_obstacle_x - 3.5, pos_obstacle_y+i*-pos_obstacle_y*2.0 + 8.5,height/2));
				}// else {
				//	b1 = new PassiveBox(odeHandle, osgHandle, osg::Vec3(length *2 ,length , height /*size*/));
				//	b1->setColor(Color(0.1,0.1,0.5));
				//	b1->setPose(osg::Matrix::rotate((M_PI/4), 0,0, 1) *
				//	osg::Matrix::translate(pos_obstacle_x - 8, pos_obstacle_y+i*-pos_obstacle_y*2.0 + 21,height/2));
				//}
	...
	void setup_Playground(GlobalData& global)
	{
		...
		double length_pg = 8;//28;//0.0; //45, 32, 22
		...
		playground->setPosition(osg::Vec3(3.5, -3, 0.0)); // playground positionieren und generieren
	...
	void start(const OdeHandle& odeHandle, const OsgHandle& osgHandle, GlobalData& global)
	{
		...
		if (ac_ico_robot)
		{
			...
			else
			{
				...
				Pos pos(position_x-13.5/*, +x = to left, -x = to right*/,-5.0/*y*/,0.0/*z*/);
