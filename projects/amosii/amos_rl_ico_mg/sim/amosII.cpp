    ...
    if (GoalSensor_active) {
      //the first goals
    	...
    	std::vector<RelativePositionSensor>::iterator it = GoalSensor.begin(); //we only use one goal sensor
    	 std::list<sensor> gls_val;
    	for (int i=0; i<GoalSensor.size(); i++) {
    		gls_val = it->get();
    		sensors[82+(3*i)] = gls_val.back();
		    gls_val.pop_back();
		    sensors[83+(3*i)] = gls_val.back();
		    gls_val.pop_back();
		    sensors[84+(3*i)] = gls_val.back();
		    gls_val.pop_back();
		    it++;
    	}
     ...
     /* std::list<sensor> gls_val = it->get();
      ...
      it++;
      gls_val = it->get();
      sensors[G2z_s + 3] = gls_val.back();
      gls_val.pop_back();
      sensors[G2y_s + 3] = gls_val.back();
      gls_val.pop_back();
      sensors[G2x_s + 3] = gls_val.back();
      gls_val.pop_back();*/
    }
    //------------------------Add GoalSensor by Ren-------------------
