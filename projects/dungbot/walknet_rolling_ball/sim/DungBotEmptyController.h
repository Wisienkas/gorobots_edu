...
class DungBotEmptyController : public AbstractController
{
	...
	private:
		...
		void ballstand_head( std::vector<std::vector<double>> &);
		void ballstand_nohead( std::vector<std::vector<double>> &);
		...
		int ball_state = 0;
		enum ball_state { INIT, STATE1, STATE2, STATE3 };
		bool getGroundContact_tarsus();
		bool getGroundContact_tibia();
		bool getGroundContact_femur();
