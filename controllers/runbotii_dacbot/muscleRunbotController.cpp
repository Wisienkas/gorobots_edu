...
static double changeToCPG=1;
static double switchOffSensorsReflex=0;
static double switchOffSensorsCPG=1;

// changeToCPG=0; switchOffSensorsReflex=1; = Reflex and switch off foot sensors after 700 steps
// changeToCPG=0; switchOffSensorsReflex=0; = Only Reflex
// changeToCPG=1; switchOffSensorsReflex=0; = From Reflex to CPG after 1000 steps
// changeToCPG=1; switchOffSensorsCPG=1; = From Reflex to CPG after 1000 steps and then switch off foot sensor after 1000 steps
...
: AbstractController (name,revision){
  ...
  addInspectableValue("FootLeftSensor",&fl_sensor,"FootLeftSensor");
  addInspectableValue("FootRightSensor",&fr_sensor,"FootRightSensor");
...
void MuscleRunbotController::init(int sensornumber, int motornumber, RandGen* randGen) {
  ...
  hipPlot.open("/home/dacbot/Documents/feedbackNew.dat");//change this to your directory
...
void MuscleRunbotController::step(const sensor* sensors, int sensornumber, motor* motors, int motornumber) {
  ...
  if(oneCPG==true)//using only one CPG in the controller
  {
    ...
    if(switchOffSensorsReflex == 1 && steps>700)
    {
      actualAD[LEFT_FOOT] = 4000;// set foot sensor = zero
      actualAD[RIGHT_FOOT] = 4000; // set foot sensor = zero
      cout<<"Foot sensors are off = 0"<< std::endl;
    }
    ...
    DinLeft->setEnable((steps>1000 && changeToCPG == 1),motors[0],  left_foot_sensor ,right_foot_sensor);// generates enable output
    enable=DinLeft->getEnable();//set enable
    std::cout << " enable: " << enable << std::endl;

    if(enable==1)
    {
      std::cout << "CPG control" << std::endl;
      if(switchOffSensorsCPG == 1 && steps>1000)
        {
          actualAD[LEFT_FOOT] = 4000;// set foot sensor = zero
          actualAD[RIGHT_FOOT] = 4000; // set foot sensor = zero
          cout<<"Foot sensors are off = 0"<< std::endl;
    ...
    {
      std::cout << "Reflex control" << std::endl;
    }


    fl_sensor = (actualAD[LEFT_FOOT]>3000)?0.0:1.0;
    fr_sensor = (actualAD[RIGHT_FOOT]>3000)?0.0:1.0;

    //wrtie to file, change directory



    hipPlot << steps <<" "<<motors[0] <<" "<<motors[1] <<" "<<motors[2] <<" "<<motors[3] <<" "<<DinLeft->getCpg()->getFrequency()*2.1/0.015<< " "<<sign[0]<<" "<<motorOutput[0] << " " << motorOutput[1]<<" " << motorOutput[2]  <<" "<< motorOutput[3] <<
        " "<< actualAD[LEFT_HIP]<<" "<<actualAD[RIGHT_HIP]<<" " <<actualAD[LEFT_KNEE]<<" "<<enable<<" "<<DinLeft->getCpg()->getOut0()<<" "<<DinLeft->getCpg()->getOut0()<<" "<<actualAD[RIGHT_KNEE]<<" "<<left_foot_sensor<< " " << right_foot_sensor<<std::endl ;
