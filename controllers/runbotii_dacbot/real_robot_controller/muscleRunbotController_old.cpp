...
hipPlot.open("/home/dacbot/Documents/dacbot.dat");//change this to your directory
	...
	if(oneCPG==true)//using only one CPG in the controller
{
	...
	std::cout<<"sensors 6 : "<<left_foot_sensor<<std::endl;
	std::cout<<"sensors 6 : "<<right_foot_sensor<<std::endl;
	...
	//motorOutput=nnet->update(actualAD,steps);
	 ...
	 // else if(steps >= 5000 && steps <7000)
      ...
      //wrtie to file, change directory
          ...
          hipPlot <<motors[0] <<" "<<motors[1] <<" "<<motors[2] <<" "<<motors[3] <<" "<< steps <<" "<<motorOutput[0] <<" "<<DinLeft->getCpg()->getFrequency()*2.1/0.015<< " "<<sign[0]<<" "<<motorOutput[0] << " " << motorOutput[2]<<" " << motorOutput[3]  <<
...
*/
		      ...
		      DinLeft->setEnable(false,motors[0],  left_foot_sensor ,right_foot_sensor);
