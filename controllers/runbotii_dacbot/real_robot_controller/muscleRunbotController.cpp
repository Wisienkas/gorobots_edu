...
static double switchOffSensors=0;

// changeToCPG=0; switchOffSensors=1; = Reflex and switch off foot sensors after 300 steps
// changeToCPG=0; switchOffSensors=0; = Only Reflex 
// changeToCPG=1; switchOffSensors=0; = From Reflex to CPG after 1000 steps
...
void MuscleRunbotController::step(const sensor* sensors, int sensornumber, motor* motors, int motornumber) {
  ...
  if(oneCPG==true)//using only one CPG in the controller
  {
    ...
    if(switchOffSensors == 1 && steps>300)
    {
      actualAD[LEFT_FOOT] = 4000;// set foot sensor = zero
      actualAD[RIGHT_FOOT] = 4000; // set foot sensor = zero
      cout<<"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<< std::endl;
    }
