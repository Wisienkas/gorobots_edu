...
NeuralLocomotionControlAdaptiveClimbing::NeuralLocomotionControlAdaptiveClimbing(){
  ...
  switchon_reflexes = false;// true;//true;//1;// true==on after learning or when uses learned weights, false == off during learning

  //Switch on pure foot signal
  switchon_purefootsignal = false;//true; // 1==on using only foot signal, 0 == using forward model & foot signal
  ...
  switchon_less_reflexes = false;//true;// = very high leg extension during searching reflex
...
std::vector<double> NeuralLocomotionControlAdaptiveClimbing::step_nlc(const std::vector<double> in0, const std::vector<double> in1){//, bool Footinhibition){
  ...
  //Control_input = 0.15;//slow stable Tetrapod OK USED
  ...
  //
  //    m_r0_t_old = m_pre.at(TR0_m);
  //    m_r1_t_old = m_pre.at(TR1_m);
  //    m_r2_t_old = m_pre.at(TR2_m);
  //    m_l0_t_old = m_pre.at(TL0_m);
  //    m_l1_t_old = m_pre.at(TL1_m);
  //    m_l2_t_old = m_pre.at(TL2_m);
  //
  //    if(!initialized)
  //    { i_text_loop = 0;
  ...
  //
  //
  //    m_pre.at(TR0_m) = 2*m_r0_t.at(ii);// m_reflex.at(TR0_m);
  //    m_pre.at(TR1_m) = 2*m_r1_t.at(ii);//m_reflex.at(TR1_m);
  //    m_pre.at(TR2_m) = 2*m_r2_t.at(ii);//m_reflex.at(TR2_m);
  //
  //    m_pre.at(TL0_m) = 2*m_l0_t.at(ii);//m_reflex.at(TL0_m);
  //    m_pre.at(TL1_m) = 2*m_l1_t.at(ii);//m_reflex.at(TL1_m);
  //    m_pre.at(TL2_m) = 2*m_l2_t.at(ii);//m_reflex.at(TL2_m);
  //
  //    double up = 0.0;//0.6;
  //
  //    if(m_pre.at(TR0_m)>m_r0_t_old)
  //      m_pre.at(CR0_m) = m_pre.at(TR0_m)+0.5;
  //    else
  //      m_pre.at(CR0_m) = up;
  //
  //    if(m_pre.at(TR1_m)>m_r1_t_old)
  //      m_pre.at(CR1_m) = m_pre.at(TR1_m)+1.5;
  //    else
  //      m_pre.at(CR1_m) = up;
  //
  //    if(m_pre.at(TR2_m)>m_r2_t_old)
  //      m_pre.at(CR2_m) = m_pre.at(TR2_m)+1.5;
  //    else
  //      m_pre.at(CR2_m) = up;
  //
  //    if(m_pre.at(TL0_m)>m_l0_t_old)
  //      m_pre.at(CL0_m) = m_pre.at(TL0_m)+1.5;
  //    else
  //      m_pre.at(CL0_m) = up;
  //
  //    if(m_pre.at(TL1_m)>m_l1_t_old)
  //      m_pre.at(CL1_m) = m_pre.at(TL1_m)+1.5;
  //    else
  //      m_pre.at(CL1_m) = up;
  //
  //    if(m_pre.at(TL2_m)>m_l2_t_old)
  //      m_pre.at(CL2_m) = m_pre.at(TL2_m)+1.5;
  //    else
  //      m_pre.at(CL2_m) = up;
  //
  //    m_pre.at(FR0_m) = -1;//m_reflex.at(FR0_m);
  //    m_pre.at(FR1_m) = -1;//m_reflex.at(FR1_m);
  //    m_pre.at(FR2_m) = -1;//m_reflex.at(FR2_m);
  //
  //    m_pre.at(FL0_m) = -1;//m_reflex.at(FL0_m);
  //    m_pre.at(FL1_m) = -1;//m_reflex.at(FL1_m);
  //    m_pre.at(FL2_m) = -1;//m_reflex.at(FL2_m);
  //
  //    m_pre.at(BJ_m) =  bj_output.at(0);//m_reflex.at(BJ_m);//(m_reflex.at(TR0_m)-0.35)*5;//
  //
  //    //------------------Reading signals from Text file
  ...
  if(reading_text_testing)
  {
    ...
    //ifstream b_file ("soinn/AMMC_Neurallocomotion_0.02_R1R2R3L1L2L3.txt");//( "Test1.txt" );
    //ifstream b_file ("soinn/AMMC_Neurallocomotion_0.05_L1_R1wL2_R2wL3_R3.txt");//( "Test1.txt" );
    ifstream b_file ("soinn/AMMC_Neurallocomotion_0.11_R1L1R2L2R3L3.txt");//( "Test1.txt" );
    //ifstream b_file ("soinn/AMMC_Neurallocomotion_0.18_R1wL2wR3_L1wR2wL3.txt");//( "Test1.txt" );

    //Original
    //ifstream b_file ("neurallocomotionOriginal/Neurallocomotion_0.02_R1R2R3L1L2L3.txt");//( "Test1.txt" );
    //ifstream b_file ("neurallocomotionOriginal/Neurallocomotion_0.05_L1_R1wL2_R2wL3_R3.txt");//( "Test1.txt" );
    //ifstream b_file ("neurallocomotionOriginal/Neurallocomotion_0.11_R1L1R2L2R3L3.txt");//( "Test1.txt" );
    //ifstream b_file ("neurallocomotionOriginal/Neurallocomotion_0.18_R1wL2wR3_L1wR2wL3.txt");//( "Test1.txt" );
    ...
    m_r0_t_old = m_pre_cr.at(CR0_m); //m.at(TR0_m);
    m_r1_t_old = m_pre_cr.at(CR1_m); //m.at(TR1_m);
    m_r2_t_old = m_pre_cr.at(CR2_m); //m.at(TR2_m);
    m_l0_t_old = m_pre_cl.at(CL0_m); //m.at(TL0_m);
    m_l1_t_old = m_pre_cl.at(CL1_m); //m.at(TL1_m);
    m_l2_t_old = m_pre_cl.at(CL2_m); //m.at(TL2_m);
    ...
    m_pre_cr.at(CR0_m) = (m.at(TR0_m)*0.98)*0.5+0.6;//+0.5;
    m_pre_cr.at(CR1_m) = ((m.at(TR1_m)+0.5)*0.85)*0.5+0.6;//+1.5;//1.5;
    m_pre_cr.at(CR2_m) = (m.at(TR2_m)+0.85)*0.5+0.6;//+1.5;//1.5;
    m_pre_cl.at(CL0_m) = (m.at(TL0_m)*0.98)*0.5+0.6;//+0.5;//1.5;//1.5;
    m_pre_cl.at(CL1_m) = ((m.at(TL1_m)+0.5)*0.85)*0.5+0.6;//+1.5;//1.5;
    m_pre_cl.at(CL2_m) = (m.at(TL2_m)+0.85)*0.5+0.6;//+1.5;//1.5;


    if(liftbody_up_reading_text == true)
    {
      up = 0.5;//0.0 //high;//0.5 //middle;
      fti_fix = -0.75;//-0.1//high;//-0.75//middle;
    }

    m.at(CR0_m) = m_pre_cr.at(CR0_m);
    if(m_pre_cr.at(CR0_m)<m_r0_t_old)//||m.at(TR0_m)<up)
      m.at(CR0_m) = up;

    m.at(CR1_m) = m_pre_cr.at(CR1_m);
    if(m_pre_cr.at(CR1_m)<m_r1_t_old)//||m.at(TR1_m)<up)
      m.at(CR1_m) = up;

    m.at(CR2_m) = m_pre_cr.at(CR2_m);
    if(m_pre_cr.at(CR2_m)<m_r2_t_old)//||m.at(TR2_m)<up)
      m.at(CR2_m) = up;

    m.at(CL0_m) = m_pre_cl.at(CL0_m);
    if(m_pre_cl.at(CL0_m)<m_l0_t_old)
      m.at(CL0_m) = up;

    m.at(CL1_m) = m_pre_cl.at(CL1_m);
    if(m_pre_cl.at(CL1_m)<m_l1_t_old)
      m.at(CL1_m) = up;

    m.at(CL2_m) = m_pre_cl.at(CL2_m);
    if(m_pre_cl.at(CL2_m)<m_l2_t_old)
      m.at(CL2_m) = up;


    m.at(FR0_m) = fti_fix;//m_reflex.at(FR0_m);
    m.at(FR1_m) = fti_fix;//m_reflex.at(FR1_m);
    m.at(FR2_m) = fti_fix;//m_reflex.at(FR2_m);

    m.at(FL0_m) = fti_fix;//m_reflex.at(FL0_m);
    m.at(FL1_m) = fti_fix;//m_reflex.at(FL1_m);
    m.at(FL2_m) = fti_fix;//m_reflex.at(FL2_m);
