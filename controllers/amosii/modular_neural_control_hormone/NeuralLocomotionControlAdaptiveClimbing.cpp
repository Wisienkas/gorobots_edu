...
void NeuralLocomotionControlAdaptiveClimbing::init(int aamosVersion,bool mMCPGs,bool mMuscleModelIsEnabled)
{
	...
	if(MCPGs == true)
		lift_value= 40.0; // for MCPGs with foot contact sensory feedback (Subhi controller)
	else
		lift_value= 0.0;
...
std::vector<double> NeuralLocomotionControlAdaptiveClimbing::step_nlc(const std::vector<double> inreflex,
	 ...
	 *******************************************************************************/
	if (1==0)//muscleModelIsEnabled) // Muscle model is utilized
	{
	...
	else
	{
	       ...
	       m.at(TR1_m) = m_reflex.at(TR1_m);
	        ...
	        m.at(TL0_m) = m_reflex.at(TL0_m)*0.70;
	        m.at(TL1_m) = m_reflex.at(TL1_m)*0.70;
	        m.at(TL2_m) = m_reflex.at(TL2_m)*0.70;//-1.0

	     //   m.at(CL0_m) = 1;//m_reflex.at(CL0_m);
	        m.at(CL0_m) = m_reflex.at(CL0_m)*1.3-0.1;
	        m.at(CL1_m) = m_reflex.at(CL1_m)*1.3-0.1;
	        m.at(CL2_m) = m_reflex.at(CL2_m)*1.5-0.1;
	        //m.at(CL2_m) = 1;//m_reflex.at(CL2_m);

	        m.at(CR0_m) = m_reflex.at(CR0_m)*1.3-0.1;
	        m.at(CR1_m) = m_reflex.at(CR1_m)*1.3-0.1;
	        m.at(CR2_m) = m_reflex.at(CR2_m)*1.5-0.1;
	    ...
	    //    m.at(CR2_m) = 1;//m_reflex.at(CR2_m);
	        ...
	        m.at(FR0_m) = -0.8+lift_value*0.01-offset;//m_reflex.at(FR0_m);
	        m.at(FR1_m) = -0.7+lift_value*0.01-offset;//m_reflex.at(FR1_m);
	        m.at(FR2_m) = -0.9+lift_value*0.01-offset;//m_reflex.at(FR2_m);

	        //m.at(FR2_m) = 1;//m_reflex.at(FR2_m);

	        m.at(FL0_m) = -0.8+lift_value*0.01-offset;//m_reflex.at(FL0_m);
	        m.at(FL1_m) = -0.7+lift_value*0.01-offset;//m_reflex.at(FL1_m);
	        m.at(FL2_m) = -0.9+lift_value*0.01-offset;//m_reflex.at(FL2_m);
	        //std::cout<<"offset"<<offset<<"\n"<<endl;
	        m.at(BJ_m) =  m_reflex.at(BJ_m)-0.05;
