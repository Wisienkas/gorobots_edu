 ...
 */
class AmosMG : public AbstractController {
...
public:
	...
	double low_pass_dis_old[NUMBER_OF_BALLS];
	double low_pass_dis[NUMBER_OF_BALLS];

	bool terminate_immediatly;
	AmosMgRobotState robotstate;

	double old_ico_value;
	double ico_out;
	double ico_weight;
	double real_v;
	double Vt_old;// Initial only one time
	double rt_old;


	vector<double> old_x;
	vector<double> x_lowpass;

	double temp_nearby_distance;
	...
	double prediction_observer;
	double reflex_observer;
...
//	ofstream outFilevtcurveY;
	...
	void signal_filter(const sensor* x_);
	void inner_controller(const sensor* x_, int number_sensors, motor* y_, int number_motors);
	AmosMG(const AmosMgConf& conf = getDefaultConf());
  ...
  virtual paramlist getParamList() const;
	...
	static AmosMgConf getDefaultConf(){
		AmosMgConf c;
	...
	void submit_sample(double* input_old, double td_error, double pure_error, double actor_update_value);
	...
	double get_exploration(double Vt);
  ...
  void inputSensorSignals(const sensor* x_);
	...
	void get_RBF_network_out(double* in, double* out);


	void update_critic_actor(double* input_old, double td_error
