...
void walknetcontroller::coordinatingInfluences( const sensor* sensor )
{
	...
	coordinateRule1( sensor );
	coordinateRule2( sensor );
	coordinateRule3( sensor );
	//coordinateRule4( sensor );
}

void walknetcontroller::coordinateRule1( const sensor* sensor ){
	for( int i = 5; i>=0; i--)
	{
		switch (i) {
			case 0:
...
void walknetcontroller::coordinateRule2( const sensor* sensor ){
	for( int i = 5; i>=0; i--)
	{
		switch (i) {
			case 0:
			...
			case 2:
				...
				if( sensor[154] < 0.8 ){continue;} //If tilted, move on.
				if(separateLegs[i].touch_down == true){
					separateLegs[i].touch_down = false;
...
void walknetcontroller::coordinateRule3( const sensor* sensor ){
	for( int i = 5; i>=0; i--)
	{
		if( sensor[154] < 0.8 ){
			switch (i) {
