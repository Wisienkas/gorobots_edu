 ...
 **************************************************************************/
//General information: Nimm - complex environment
/*************************************************************************
 ...
 *
 * Parameters:
 * 		The actor's learning rate = 0.025
 * 		The critic's learning rate = 0.06
 * 		The discount factor = 0.9999
 * 		exploration scale factor = 23
...
: AbstractController("NimmComplex", "$Id: "), conf(_conf)
{
	...
	addInspectableValue("dis1",&distances[0],"dis1");
	addInspectableValue("dis2",&distances[1],"dis2");
...
NimmComplex::~NimmComplex()
{



}
void NimmComplex::init(int sensornumber, int motornumber, RandGen* randGen)
{
	stepnumber_t = 0;
	++current_epoch;
	if(!initialized) //-----------------------------------------------(FRANK)
	{
		...
		{
			si = 1.0;//10.0;+++
			rate_policy = 0.025; //0.001;//0.01;//0.01;// learning rate of actor
			rate_valuefunction = 0.06;// learning rate_critic of weights from hidden RBF neurons to its output neuron (critic)
...
void NimmComplex::calculateAnglePositionFromSensors(const sensor* x_)
{
	int i=0;
...
//calculate distances
void NimmComplex::calculateDistanceToGoals(const sensor* x_)
{
...
void NimmComplex::inputSensorSignals(const sensor* x_)
{
	xt_ir = input_ir_s;
...
void NimmComplex::get_ir_signals(const sensor* x_)
{
...
void NimmComplex::print_to_cout(int steps_period)
{
...
void NimmComplex::copy_input(double* inputs, double* inputs_old)
{
	for (int i=0; i<XDIM; i++) {
...
void NimmComplex::step(const sensor* x_, int number_sensors, motor* y_, int number_motors)
{
...
void NimmComplex::controller_core(const sensor* x_, int number_sensors, motor* y_, int number_motors)
{
	...
	/********************************************************SENSORY INPUTS*******************/
	/*xt[0] = low_pass_angles[5];
	xt[1] = low_pass_angles[6];
	xt[2] = low_pass_angles[7];*/

	xt[0] = distances[4];
	xt[1] = distances[5];
	xt[2] = low_pass_angles[6];
	...
	if((ntrial < MAX_TRIAL)&&(!failure_flag) && (stepnumber_t > 10))
	{
		...
		robotstate.submit_sample(input_old, td_error, exploration_old, p_error);
		//Update section
...
void NimmComplex::stepNoLearning(const sensor* x_, int number_sensors,
		motor* y_, int number_motors)
{

}

double NimmComplex::sigmoid(double num)
{
	return 1./(1.+exp(-num));
}

double NimmComplex::tanh(double num)
{
	return 2./(1.+exp(-2*num))-1.;
}
//reward function
double NimmComplex::reward_function(double *x /*state*/, double *u /*action*/)
{
	/*bool far = true;
	for (int i=0; i<NUMBER_OF_BALLS; i++)
		if (distances[i] < 0.02)
...
int NimmComplex::check_limit(double *x)//, double irr_back, double irl_back)
{
...
//	Initial RBF network
void NimmComplex::init_funcapprox()
{

	int num_unit_1 = 5;//6;
	int num_unit_2 = 7;//5;

	double max_range1 = +M_PI;// + 0.3;
	double min_range1 = -M_PI;// - 0.3;
	...
	xmax[_ias0] = 2.5;//max_range1;
	xmin[_ias0] = 0;//min_range1;

	xmax[_ias1] = 2.5;//max_range1;
	xmin[_ias1] = 0;//min_range1;

	xmax[_ias2] = max_range1 + 0.2;
	xmin[_ias2] = min_range1 - 0.2;
...
void NimmComplex::get_v_action_pair_from_RBF_network(double* xt, double* rbf_out) {
	double u_tmp1[2];
	u_tmp1[0] = 0.0;
	u_tmp1[1] = 0.0;
	...
	rbf_out[0] = u_tmp1[0];
	rbf_out[1] = clip (u_tmp1[1], -0.04, 0.04);
	...
	if (!USE_SAVED_RBF) {
		...
		//get noise
	#define		exploration_gain 0.9//0.95//0.9997
		double exp = robotstate.get_exploration(real_v, exploration_gain);
		//exp = clip(exp, -0.01, 0.01);
		...
		exploration_out = clip(exploration_out, -0.4, 0.4); // maximal control signal
		rbf_out[1] = robotstate.bind_exploration(u_tmp[0], exploration_out, ico_active_param);
		rbf_out[1] = clip(rbf_out[1], -0.4, 0.4);
...
void NimmComplex::update_rbf_network(double* input_old, double td_error, double td_actor, double rate_valuefunction, double p_error, double rate_policy) {
...
//////////////////////////////////////////////////////////////////////////////
void NimmComplex::output_policy(double *x /*in*/, double *u /*return*/)
{
	/*///get actual actor value
	double u_tmp[UDIM];
	u_tmp[0] = 0.0;
...
#define		exploration_gain 0.99//0.9997
	exploration_out = robotstate.get_exploration(real_v, exploration_gain);
	if (ico_out != 0)
		exploration_out = ico_out;
	u[0] = robotstate.bind_exploration(u_tmp[0], exploration_out);
	actual_out = u[0];*/
}
//Value function calculation
double NimmComplex::value_function(double *x /*in*/)
{
	double Value;
...
void NimmComplex::update_valuefunction_trace(double *x)
{
	ngnet->incsbox_trace(&VALUE, x, lambda_v /*trace, set to 0.0 = no trace*/, &nbasis /*number of hidden neurons*/);

}
//UPDATE VALUE FUNCTION WEIGHTS
void NimmComplex::update_valuefunction(double *x, double td/*TDerror*/, double rate)
{
...
void NimmComplex::update_policy_actor(double td_error, double rate, double* input)
{
	//int i;
