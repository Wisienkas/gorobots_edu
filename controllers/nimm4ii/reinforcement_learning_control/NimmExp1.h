...
class NimmExp1 : public AbstractController {
...
//	ofstream outFilevtcurveY;
	...
	double p_error;
	double real_v;
	bool terminate_after_this_step;

	NimmExp1RobotState robotstate;
	double distances [NUMBER_OF_BALLS];

	double taken_rewards[NUMBER_OF_BALLS];
	double reached_goals[NUMBER_OF_BALLS];
	...
	NimmExp1(const NimmExp1Conf& conf = getDefaultConf());
  ...
  virtual paramlist getParamList() const;
	...
	void controller_core(const sensor* x_, int number_sensors, motor* y_, int number_motors);
	static NimmExp1Conf getDefaultConf(){
		NimmExp1Conf c;
