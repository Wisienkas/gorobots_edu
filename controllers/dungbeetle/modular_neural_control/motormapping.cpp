...
else if (amosVersion==2) //AMOSIIv2
{
  if (TC(motor)) {
   ...
   std::cout << endl << "out of motor number range (no return value)" << endl << endl; //warning when none of the if cases matches
	...
	{
		cout<<"Error: you should select one of two AMOS types, i.e., AMOSv1=1 or AMOSv2=2";
	return 0;
	}
}
...
double Mapping::getReflex(int motor, double motorvalue, double fmodel_error,int amosVersion,double offset_lift) {
